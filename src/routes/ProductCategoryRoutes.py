...
async def get_all_categories_route():
    ...
    try:
        return await ProductoCategoryController.get_all_categories()
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e))

@router_ProductsCategory.get("/categories/{category_id}")
async def get_category_by_id_route(category_id: int):
    """Obtiene una categoría por su ID."""
    category = await ProductoCategoryController.get_category_by_id(category_id)
    if not category:
